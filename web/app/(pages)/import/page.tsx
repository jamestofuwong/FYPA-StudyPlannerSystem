      ...
      <div className={styles.splitLayout}>
        <div className={styles.splitMain}>
          ...
          >
            ...
            <div style={{ fontSize: 13, color: 'var(--text-primary)', marginBottom: 4 }}>Drag &amp; Drop Study Planner PDF here</div>
            <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>or click to browse files · Supports PDF, scanned PDF</div>
          </div>
          ...
          <div className={styles.card}>
            ...
            <div className={styles.formGroup}>
              ...
              <input type="range" min={50} max={99} defaultValue={80} style={{ width: '100%', accentColor: 'var(--active-highlight)' }} />
              <div
                style={{
                  display: 'flex',
            ...
            <div className={styles.btnGroup}>
              <button className={styles.btnPrimary} onClick={() => showToast('OCR processing started...', 'info')}>
                🔍 Parse PDF (OCR)
              </button>
              <button className={styles.btnSuccess} onClick={() => showToast('Study planner updated successfully!', 'success')}>
                ✅ Update Planner
              </button>
              <button className={styles.btnSecondary} onClick={() => showToast('Import session cleared.', 'info')}>
                🗑️ Clear
              </button>
            </div>
          </div>
              ...
              <tbody>
                {[
                  ...
                  <tr key={r.code}>
                    ...
                    <td>{r.yr}</td>
                    <td>
                      <span className={`${styles.badge} ${styles.badgeRed}`}>{r.type}</span>
                    </td>
                    <td>{r.pre !== '—' ? <code className={styles.code}>{r.pre}</code> : '—'}</td>
                    <td>
                      <span className={`${styles.badge} ${styles[r.cls as keyof typeof styles]}`}>{r.conf}%</span>
                    </td>
                  </tr>
                ))}
              </tbody>
