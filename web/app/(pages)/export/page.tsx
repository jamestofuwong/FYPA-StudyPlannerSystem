            ...
            {[
              {
                ...
                desc: 'Exports current student major detection results, match percentages, and completion stats.',
                btnCls: 'btnPrimary',
                msg: 'Dashboard Report exported successfully!',
              },
              {
                icon: '📋',
                title: 'Advisory Plan',
                desc: 'Full advisory plan per student with recommended unit paths, missing units and alternatives.',
                btnCls: 'btnPrimary',
                msg: 'Advisory Plans exported successfully!',
              },
              {
                icon: '👥',
                title: 'Student Data',
                desc: 'All student enrollment data scraped from portal including GPA, units, and cohort details.',
                btnCls: 'btnPrimary',
                msg: 'Student Data exported successfully!',
              },
            ].map((c) => (
              <div
                key={c.title}
              ...
              >
                ...
                <button
                  className={styles[c.btnCls as keyof typeof styles]}
                  style={{ width: '100%', justifyContent: 'center' }}
                  onClick={() => showToast(c.msg, 'success')}
                >
                  📥 Export
                </button>
              </div>
            ...
            {[
              {
                ...
                desc: 'Breakdown of all students by detected major, second major possibilities, and confidence scores.',
                btnCls: 'btnPrimary',
                msg: 'Major Analysis exported successfully!',
              },
              {
                icon: '📄',
                title: 'Unit Planner DB',
                desc: 'Export the current study planner database — all programmes, units, prerequisites.',
                btnCls: 'btnPrimary',
                msg: 'Unit Planner DB exported successfully!',
              },
              {
                icon: '📈',
                title: 'Analytics Report',
                desc: 'Cohort-level analytics: average GPA, at-risk students, major distribution trends.',
                btnCls: 'btnPrimary',
                msg: 'Analytics Report exported successfully!',
              },
            ].map((c) => (
              <div
                key={c.title}
            ...
            <div className={styles.formGrid2}>
              ...
              <div className={styles.formGroup}>
                ...
                <input
                  ...
                  defaultValue="C:\\Users\\JoeLee\\Desktop\\SUMS_Exports\\"
                />
              </div>
              <div className={styles.formGroup}>
                <label className={styles.formLabel}>Include Charts in Excel</label>
                <select className={styles.formSelect}>
                  <option>Yes — Embedded Charts</option>
                  <option>No — Data Only</option>
                </select>
              </div>
            </div>
            <div className={styles.btnGroup}>
              <button className={styles.btnPrimary} onClick={() => showToast('Exporting all reports...', 'info')}>
                📦 Export All Reports
              </button>
              <button className={styles.btnSecondary} onClick={() => showToast('Output folder opened.', 'info')}>
                📁 Open Output Folder
              </button>
            </div>
          </div>
              ...
              <tbody>
                {[
                  ...
                  <tr key={r.file}>
                    ...
                    <td>
                      <span className={`${styles.badge} ${styles[r.sc as keyof typeof styles]}`}>{r.s}</span>
                    </td>
                  </tr>
                ))}
              </tbody>
