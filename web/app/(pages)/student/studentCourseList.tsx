...
import { gradeClass, groupByTerm, statusClass } from './termFunction';
      ...
      {termGroups.map(({ term, list, parsed, isEmpty }) => (
        <div key={term} className={styles.termGroup}>
          <div className={styles.termHeading}>
            <span className={isEmpty ? styles.termLabelEmpty : styles.termLabel}>{parsed.label}</span>
            {isEmpty && <span className={`${styles.badge} ${styles.badgeOrange}`}>No enrolment</span>}
            <div className={styles.termDivider} />
          </div>

          {isEmpty ? (
            <div className={styles.termEmpty}>No subjects taken this semester.</div>
          ) : (
              ...
              <table className={styles.table}>
                ...
                <tbody>
                  {list.map((c) => (
                    <tr key={c.courseId}>
                      <td>
                        <code className={styles.code}>{c.courseId}</code>
                      </td>
                      <td>{c.courseTitle}</td>
                      <td className={styles.tdRight}>{c.credits}</td>
                      <td className={styles.tdRight} style={{ color: c.creditsEarned > 0 ? 'var(--accent-green)' : 'var(--text-muted)' }}>
                        {c.creditsEarned > 0 ? c.creditsEarned : '—'}
                      </td>
                      <td>
                        {c.grade ? <span className={`${styles.badge} ${gradeClass(c.grade)}`}>{c.grade}</span> : <span className={styles.textMuted}>—</span>}
                      </td>
                      <td>
